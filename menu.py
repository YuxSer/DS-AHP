...
class Menu:
    ...
    def generate_custom_data(self, generator: XMLDataGenerator):
        """Настройка параметров генерации и запуск"""
        print("\n" + "=" * 70)
        print("НАСТРОЙКА ПАРАМЕТРОВ ГЕНЕРАЦИИ (ОПТИМИЗИРОВАННАЯ ВЕРСИЯ)")
        print("=" * 70)
        ...
        try:
            ...
            while True:
                try:
                    n_alts_input = input("\nКоличество альтернатив (1-1000): ").strip()

                    n_alts = int(n_alts_input)
                    if 1 <= n_alts <= 1000:
                        break
                    else:
            ...
            while True:
                try:
                    n_criteria_input = input("Количество критериев (1-100): ").strip()
                    n_criteria = int(n_criteria_input)
                    if 1 <= n_criteria <= 100:
                        break
                    else:
            ...
            while True:
                try:
                    n_experts_input = input("Количество экспертов (1-200): ").strip()
                    n_experts = int(n_experts_input)
                    if 1 <= n_experts <= 200:
                        break
                    else:
            ...
            print("  2. Убывающее (от 1.0 к 0.2)")
            print("  3. Случайное оптимизированное (рекомендуется)")

            weight_choice = input("Выберите распределение (1-3, Enter для 3): ").strip()
            ...
            output_dir = input(f"\nДиректория для сохранения (Enter для generated_xml): ").strip()
            if not output_dir:
                output_dir = "generated_xml"

            # Запускаем генерацию сразу
            print(f"\n🚀 Запуск оптимизированной генерации...")
            ...
            load = input(f"📥 Загрузить сгенерированный файл для анализа? (y/n): ").strip().lower()
